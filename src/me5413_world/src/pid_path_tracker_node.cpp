...
#include "me5413_world/pid_path_tracker_node.hpp"
...
void dynamicParamCallback(const me5413_world::pid_path_trackerConfig& config, uint32_t level)
{
  // Common Params
...
PidPathTrackerNode::PidPathTrackerNode() : tf2_listener_(tf2_buffer_)
{
  f = boost::bind(&dynamicParamCallback, _1, _2);
  server.setCallback(f);

  this->sub_robot_odom_ = nh_.subscribe("/gazebo/ground_truth/state", 1, &PidPathTrackerNode::robotOdomCallback, this);
  this->sub_local_path_ = nh_.subscribe("/me5413_world/planning/local_path", 1, &PidPathTrackerNode::localPathCallback, this);
...
void PidPathTrackerNode::localPathCallback(const nav_msgs::Path::ConstPtr& path)
{
...
void PidPathTrackerNode::robotOdomCallback(const nav_msgs::Odometry::ConstPtr& odom)
{
...
double PidPathTrackerNode::computeStanelyControl(const double heading_error, const double cross_track_error, const double velocity)
{
...
geometry_msgs::Twist PidPathTrackerNode::computeControlOutputs(const nav_msgs::Odometry& odom_robot, const geometry_msgs::Pose& pose_goal)
{
  // Heading Error
...
{
  ros::init(argc, argv, "pid_path_tracker_node");
  me5413_world::PidPathTrackerNode pid_path_tracker_node;
